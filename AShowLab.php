					...
					jQuery.ajax({
						...
						success:function(data)
						{
							if(data.result == 'true')
							{
								...
								$('#boCode').prop('disabled',false);
								
								
							}else
							{	
								alert('เลขที่แบบฟอร์มซ้ำ');
								$('#boCode').prop('disabled',true);
	...
	<body>
		...
		<table id='myTables' width='100%' cellspacing='10' cellpadding='5'>
			...
			<?php
				...
				echo "<tr>
						<td colspan='3'><a href='Amain.php'>ระบบขอใช้ห้องปฏิบัติการวิเคราะห์อาหารสัตว์ > เจ้าหน้าที่</a> > <a href='AGetLabT.php?statusLab=$statusLab&pageNo=$pageNo'>เอกสารขอใช้ห้องปฏิบัติการจากอาจารย์ที่อนุมัติ</a> > ตรวจสอบข้อมูล</td>
					</tr>";
			?>
			...
			<form action='AShowLabQuery.php' method='POST'>		
								...
								<?php
									...
									<tr>
										<td colspan='2' align='right'>
											เลขที่&nbsp;
											<input type='text' name='labNo' id='labNo' style='width:50px;' value='$recnumSearchLab[2]' onkeypress='return chkDigit(event.charCode)'>&nbsp;.&nbsp;
										...
										<td colspan='2'>
											<table id='tbSim' border='1' width='100%' cellpadding='5'>
												<tr>
													...
													<th width='20%'>ค่าที่ต้องการวิเคราะห์</th>
													<th width='25%'>ชนิดตัวอย่าง</th>
													<th width='25%'>คำนวณ</th>
													...
													while($recnumSearchAna = mysqli_fetch_array($resultSearchAna))
													{
														...
														if($getSimName!='')
														{
															...
															echo "<tr>
																	...
																	<td>&nbsp;".$getSimName."</td>
																	<td align='center'>".number_format($getSumSim)." x $recnumSearchLab[10] x ".number_format($priceCharge)."</td>
...
<!-- -->
			...
			<!---  -->
				...
				<!---  -->
				<tr>
					<td colspan='2'>ความคิดเห็นเจ้าหน้าที่ห้องปฏิบัติการ &nbsp;:</td>
				<tr>	
					<td colspan='2'>
					<?php 
						echo "<textarea rows='10' cols='177' name='offCM' >".$recnumSearchLab[19]."</textarea>";
					?>
					</td>
				</tr>
				...
				<!-- -->
				<tr>
					<td>กรณีอนุมัติขอใช้ห้องปฏิบัติการส่งข้อความถึงหัวหน้าภาควิชา &nbsp;&nbsp;:&nbsp;&nbsp;
						<select name='boCode' id='boCode'>
							<option value='-1'>เลือกหัวหน้าภาควิชาเมื่ออนุมัติ</option>
					<?php
				
						$sqlSearchBoMember = " select * from member where memlevel='4';";
				...
				<tr>
					<td colspan='2'>
						...
						<div align='center'>
							<button id='btYes' class='1' disabled style='background-color:#e7e7e7;'>อนุมัติ/ส่งเอกสารไปยังหัวหน้าภาควิชา</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<button id='btNo' class='2'>ไม่อนุมัติ</button>
						</div>
						<?php 
							echo "<input type='hidden' name='teaName' value='".$teaName."'>";
						?>
