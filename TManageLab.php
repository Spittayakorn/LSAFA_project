	...
	<script>
		...
		$(document).ready(function(){
			...
			$('#myTable').on('click','#lookLab',function(){
				...
				window.open('TShowLab.php?labCode='+labCode+'&statusLab='+statusLab+'&pageNo='+pageNo,'_self');
			...
			$('#myTable').on('click','#status',function(){
				...
				window.open('TShowStatus.php?labCode='+labCode+'&statusLab='+statusLab+'&pageNo='+pageNo,'_self');
...
<body>
	...
	<table id='myTables' width='100%' cellspacing='10' cellpadding='5' border='0'>
		<tr>
			<td colspan='3'><a href='Tmain.php'> ระบบขอใช้ห้องปฏิบัติการวิเคราะห์อาหารสัตว์ > อาจารย์</a> > เอกสารขอใช้ห้องปฏิบัติการจากนักศึกษา </td>
		</tr>
		...
		<tr>
			...
			?>
				<table style='table-layout: fixed;border:none;' width='100%' border='0' cellpadding='10'>
					<tr>
						<td>สถานะแบบฟอร์ม : <a href='TManageLab.php?statusLab=0' align='center'>รออนุมัติ</a>&nbsp;/&nbsp;<a href='TManageLab.php?statusLab=1' align='center'>อนุมัติ</a>&nbsp;/&nbsp;<a href='TManageLab.php?statusLab=2' align='center'>ไม่อนุมัติ</a></td>
						<td></td>
					</tr>
				</table>
			</td>
	...
	<table id='myTable' align='center' cellpadding='5' width='100%'>
		...
		<?php
			...
			//-------------------------
			$sqlShowAnaList =  "select * from lab where teaCode='$memCode' and send='1' and teaStatus='$statusLab' ";
			...
			if($numRowReal == 0)
			{
				...
				{
					echo "<script>
							window.open('TManageLab.php?statusLab=$statusLab&pageNo=$previousPage','_self');
						  </script>";
				}
			...
			while($recnumSearchLab = mysqli_fetch_array($resultShowAnaList))
			{
				...
				if($recnumSearchLab[14]=='1' || $recnumSearchLab[14]=='2')
				{
...
<!--- -->
		<tr>
			<td align='center'>
				...
				if($previousPage)
				{
					echo "<a href='TManageLab.php?statusLab=$statusLab&pageNo=$previousPage'><<</a>";
				}
				for($i=1;$i<=$totalPage;$i++)
				{
					...
					{
						echo "[&nbsp;<a href='TManageLab.php?statusLab=$statusLab&pageNo=$i'>$i</a>&nbsp;]";
					}else
					{
						if($i==$pageNo)
						{
				...
				if($pageNo != $totalPage)
				{
					echo "<a href='TManageLab.php?statusLab=$statusLab&pageNo=$nextPage'>>></a>";
				}
				?>
			</td>
		</tr>
