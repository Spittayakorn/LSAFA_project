...
//---
			...
			$('#labNo').on('input',function(){
					...
					labNoVal = $('#labNo').val();
					labCodeVal = $('#labCode').val();
					jQuery.ajax({
						
						type:'GET',
						url: 'getLabNoEdit.php',
						data: {labNo: labNoVal,labCode: labCodeVal},
						success:function(data)
						{
							if(data.result == 'true')
							{
...
<body>
	...
	<table id='myTables' width='100%' cellspacing='10' cellpadding='5'>
		<tr>
			<td colspan='3'><a href='Amain.php'> ระบบขอใช้ห้องปฏิบัติการวิเคราะห์อาหารสัตว์ > เจ้าหน้าที่ห้องปฏิบัติการ</a> > <a href='AManageLab.php'>รายการแบบฟอร์มขอใช้ห้องปฏิบัติการวิเคราะห์อาหารสัตว์</a> > แก้ไขขอใช้ห้องปฏิบัติการ</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
		</tr>
	...
	<form action='AEditQueryLab.php' method='POST' onsubmit='Javascript:return chkAllData()'>
			...
			<?php
				echo "
					<tr>
						...
						<td>
							<input type='text' name='labNo' id='labNo' style='width:50px;background-color:white;text-align:right;' value='$recnumSearchLabCode[2]' onkeypress='return chkNumberr(event.charCode)'>.
							<input type='text' name='labYear' id='labYear' style='width:50px; border:none;' value='$recnumSearchLabCode[3]' readonly>	
						</td>
					</tr>
			...
			<br><br>
				...
				<tr>
					...
					<td>
						...
						<option value='-1'>กรุณาเลือกผู้บริหารที่ต้องการส่ง</option>
					<?php
						...
						{
					?>
							<option value= <?php echo $recnumSearchTeaMember[0]; ?> <?php if($recnumSearchLabCode[24]==$recnumSearchTeaMember[0]) echo "selected"; ?>><?php echo $recnumSearchTeaMember[4]; ?></option>";
