...
<html>
<head>
	<title>จัดการประเภทนักวิจัย</title>
	<meta charset='UTF-8'>
		<style>
		body {
			font-family: sarabun;
	...
	<script>
		$(document).ready(function(){
			...
			$('#tbAnalysis').on('click','#btEditAna',function(){
				...
				window.open("AEditCat.php?anaCode="+anaCode+'&pageNo='+page,'_self');
			...
			$('#tbAnalysis').on('click','#btDelAna',function(){
				...
				if(delItem)
				{
					...
					window.open("ADelCat.php?anaCode="+anaCode+'&pageNo='+page,'_self');
...
<body>
	...
	<table id='myTable' width='100%' cellspacing='10' cellpadding='5'>
		<tr>
			<td colspan='3'><a href='Amain.php'> ระบบขอใช้ห้องปฏิบัติการวิเคราะห์อาหารสัตว์ > เจ้าหน้าที่ห้องปฏิบัติการ</a> > จัดการรายการประเภทนักวิจัย</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
		</tr>
		<tr>
			<td>
				<div align='center'>
					<h2>จัดการรายการประเภทนักวิจัย</h2>
				</div>

				<table border='1' id='tbAnalysis' align='center' cellpadding='7' width='70%'>
					<tr align='right'>
						<td colspan='3'><a href='AAddcat.php'>เพิ่มประเภทนักวิจัย</a></td>
					</tr>
					
					<tr>
						<th width='10%'>ลำดับที่</th>
						<th width='30%'>รายการประเภทนักวิจัย</th>
						<th width='30%'>จัดการประเภทนักวิจัย</th>
					</tr>
		
					<?php
						require('connectDB.php');

						$sqlShowAnaList = "select * from categorys";
						...
						$sqlShowAnaList .= " order by catCode desc limit $pageStart,$perPage;";
						$resultShowAnaList = mysqli_query($con,$sqlShowAnaList);
						...
						if($numRowReal == 0)
						{
							...
							{
								echo "<script>
									window.open('AManageCat.php?pageNo=$previousPage ','_self');
								</script>";
							}
						...
						while($recnumShowAnalist = mysqli_fetch_array($resultShowAnaList))
						{
							...
							echo "<tr>
									...
									<td style='text-align:center;'>
										<button id='btDelAna' >ลบประเภทนักวิจัย</button>&nbsp;&nbsp;|&nbsp;
										<button id='btEditAna'>แก้ไขประเภทนักวิจัย</button>
									</td>
								</tr>";	
						}
					?>
		...
		<tr>
			<td align='center'>
				...
				if($previousPage)
				{
					echo "<a href='AManageCat.php?pageNo=$previousPage '><<</a>";
				}
				for($i=1;$i<=$totalPage;$i++)
				{
					...
					{
						echo "[&nbsp;<a href='AManageCat.php?pageNo=$i'>$i</a>&nbsp;]";
					}else
					{
						if($i==$pageNo)
						{
				...
				if($pageNo != $totalPage)
				{
					echo "<a href='AManageCat.php?pageNo=$nextPage'>>></a>";
				}
				?>
			</td>
		</tr>
