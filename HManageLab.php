	...
	<script>
		...
		$(document).ready(function(){
			...
			$('#myTable').on('click','#lookLab',function(){
				...
				window.open('HShowLab.php?labCode='+labCode+'&statusLab='+statusLab+'&pageNo='+pageNo,'_self');
			...
			$('#myTable').on('click','#status',function(){
				...
				window.open('HShowStatus.php?labCode='+labCode+'&statusLab='+statusLab+'&pageNo='+pageNo,'_self');
...
<body>
	...
	<table id='myTables' width='100%' cellspacing='10' cellpadding='5' border='0'>
		<tr>
			<td colspan='3'><a href='Hmain.php'> ระบบขอใช้ห้องปฏิบัติการวิเคราะห์อาหารสัตว์ > ผู้บริหารภาควิชา</a> > เอกสารขอใช้ห้องปฏิบัติการจากเจ้าหน้าที่ห้องปฏิบัติการ </td>
		</tr>
		...
		<tr>
			...
			?>
				<table style='table-layout: fixed;border:none;' width='100%' border='0' cellpadding='10'>
					<tr>
						<td>สถานะแบบฟอร์ม : <a href='HManageLab.php?statusLab=0' align='center'>รออนุมัติ</a>&nbsp;/&nbsp;<a href='HManageLab.php?statusLab=1' align='center'>อนุมัติ</a>&nbsp;/&nbsp;<a href='HManageLab.php?statusLab=2' align='center'>ไม่อนุมัติ</a></td>
						<td></td>
					</tr>
				</table>
			</td>
	...
	<table id='myTable' align='center' cellpadding='5' width='100%'>
		...
		<?php
			...
			//-------------------------
			$sqlShowAnaList =  "select * from lab where headCode='$memCode' and send='1' and ((teaStatus='1' and offStatus='1') or (teaStatus='0' and offStatus='1')) and boStatus='$statusLab' ";
			...
			if($numRowReal == 0)
			{
				...
				{
					echo "<script>
							window.open('HManageLab.php?statusLab=$statusLab&pageNo=$previousPage','_self');
						  </script>";
				}
			...
			while($recnumSearchLab = mysqli_fetch_array($resultShowAnaList))
			{
				...
				if($recnumSearchLab[16]=='1' || $recnumSearchLab[16]=='2')
				{
...
<!--- -->
		<tr>
			<td align='center'>
				...
				if($previousPage)
				{
					echo "<a href='HManageLab.php?statusLab=$statusLab&pageNo=$previousPage'><<</a>";
				}
				for($i=1;$i<=$totalPage;$i++)
				{
					...
					{
						echo "[&nbsp;<a href='HManageLab.php?statusLab=$statusLab&pageNo=$i'>$i</a>&nbsp;]";
					}else
					{
						if($i==$pageNo)
						{
				...
				if($pageNo != $totalPage)
				{
					echo "<a href='HManageLab.php?statusLab=$statusLab&pageNo=$nextPage'>>></a>";
				}
				?>
			</td>
		</tr>
