...
<html>
<head>
	<title>จัดการรายการประเภทวัตถุประสงค์</title>
	<meta charset='UTF-8'>
		<style>
		body {
			font-family: sarabun;
	...
	<script>
		$(document).ready(function(){
			...
			$('#tbAnalysis').on('click','#btEditAna',function(){
				...
				window.open("AEditObj.php?anaCode="+anaCode+'&pageNo='+page,'_self');
			...
			$('#tbAnalysis').on('click','#btDelAna',function(){
				...
				if(delItem)
				{
					...
					window.open("ADelObj.php?anaCode="+anaCode+'&pageNo='+page,'_self');
...
<body>
	...
	<table id='myTable' width='100%' cellspacing='10' cellpadding='5'>
		<tr>
			<td colspan='3'><a href='Amain.php'> ระบบขอใช้ห้องปฏิบัติการวิเคราะห์อาหารสัตว์ > เจ้าหน้าที่ห้องปฏิบัติการ</a> > จัดการรายการประเภทวัตถุประสงค์</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
		</tr>
		<tr>
			<td>
				<div align='center'>
					<h2>จัดการรายการประเภทวัตถุประสงค์</h2>
				</div>

				<table border='1' id='tbAnalysis' align='center' cellpadding='7' width='80%'>
					<tr align='right'>
						<td colspan='3'><a href='AAddObjective.php'>เพิ่มประเภทวัตถุประสงค์</a></td>
					</tr>
					
					<tr>
						<th width='10%'>ลำดับที่</th>
						<th width='30%'>รายการประเภทวัตถุประสงค์</th>
						<th width='40%'>จัดการประเภทวัตถุประสงค์</th>
					</tr>
		
					<?php
						require('connectDB.php');

						$sqlShowAnaList = "select * from objective";
						...
						$sqlShowAnaList .= " order by objCode desc limit $pageStart,$perPage;";
						$resultShowAnaList = mysqli_query($con,$sqlShowAnaList);
						...
						if($numRowReal == 0)
						{
							...
							{
								echo "<script>
									window.open('AManageObj.php?pageNo=$previousPage ','_self');
								</script>";
							}
						...
						{
							echo "<tr align='center' bgColor='EBF5FF'><td colspan='3'>กรุณาเพิ่มประเภทวัตถุประสงค์</td></tr>";
						}
						...
						while($recnumShowAnalist = mysqli_fetch_array($resultShowAnaList))
						{
							...
							echo "<tr>
									...
									<td style='text-align:center;'>
										<button id='btDelAna' >ลบประเภทวัตถุประสงค</button>&nbsp;&nbsp;|&nbsp;
										<button id='btEditAna'>แก้ไขประเภทวัตถุประสงค</button>
									</td>
								</tr>";	
						}
					?>
		...
		<tr>
			<td align='center'>
				...
				if($previousPage)
				{
					echo "<a href='AManageObj.php?pageNo=$previousPage '><<</a>";
				}
				for($i=1;$i<=$totalPage;$i++)
				{
					...
					{
						echo "[&nbsp;<a href='AManageObj.php?pageNo=$i'>$i</a>&nbsp;]";
					}else
					{
						if($i==$pageNo)
						{
				...
				if($pageNo != $totalPage)
				{
					echo "<a href='AManageObj.php?pageNo=$nextPage'>>></a>";
				}
				?>
			</td>
		</tr>
